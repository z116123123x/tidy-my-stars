  ...
  return (
    <div className="page">
      ...
      <section className="methods-grid">
        ...
        <article><span>04</span><h2>{t('Likely Unstar', '建議 Unstar')}</h2><p>{t('AI suggestions are presented with their direct reason and remaining classifications. Only the user decides whether to unstar.', 'AI 建議會連同直接理由與保留的分類一起呈現；只有使用者能決定是否 unstar。')}</p><dl><div><dt>{t('Sensitivity', '敏感度')}</dt><dd>{model.analysis.run.likely_unstar_sensitivity}/10</dd></div><div><dt>{t('Suggestions', '建議')}</dt><dd>{model.reviewRepositories.length}</dd></div></dl></article>
        <article><span>05</span><h2>{t('GitHub state', 'GitHub 狀態')}</h2><p>{t('This generated site is read-only. Browser review decisions stay on this device and are never sent to GitHub.', '這個產生的網站是唯讀的；瀏覽器中的複核決定只留在此裝置，絕不傳送到 GitHub。')}</p><dl><div><dt>{t('Application status', '套用狀態')}</dt><dd>{model.analysis.run.application_status}</dd></div><div><dt>{t('Analysis status', '分析狀態')}</dt><dd>{model.analysis.run.analysis_status}</dd></div></dl></article>
        <article><span>06</span><h2>{t('Limitations', '限制')}</h2><p>{t('Classification is an AI judgment grounded in the frozen evidence. A clear interface does not turn judgment into certainty.', '分類是 AI 根據凍結證據做出的判斷；介面清楚不代表判斷就成為確定事實。')}</p>{model.analysis.validation.notes.length ? <ul>{model.analysis.validation.notes.map((note) => <li key={note}>{note}</li>)}</ul> : <p>{t('No additional validation note was recorded.', '沒有其他驗證備註。')}</p>}</article>
      </section>
    </div>
  );
}
